...
function isValidPlacement(grid: Grid4x4, row: number, col: number, value: number): boolean {
...
export function normalizeGridFromJson(json: unknown): Grid4x4 | null {
  ...
  const grid: Grid4x4 = Array.from({ length: GRID_SIZE }, () => Array<CellValue>(GRID_SIZE).fill(null))
